# Draw UI Internal Integration

This is not a standalone Codex skill. It is an internal capability used by `multi-agent-dev-workflow`.

Prefer the workflow runner entry point for normal execution:
...
The runner enforces approval state, resolves run-level `config/imagegen.json`, calls `scripts/ask_draw.sh`, and records workflow events.

Use `multi-agent-dev-workflow/integrations/draw-ui/scripts/ask_draw.sh` only as the lower-level adapter when workflow-controlled, scripted local output paths are required. The script reads provider settings only from workflow `imagegen.json`.
...
1. workflow 中需要脚本化批量生成、本地固定输出路径时，使用 `multi-agent-dev-workflow/integrations/draw-ui/scripts/ask_draw.sh`。
2. provider、model、base_url、api_key_env 都从 run 级 `config/imagegen.json` 读取。
3. 多张屏幕必须串行执行，避免 provider 限流和多屏一致性漂移。
...
API Key 只通过 workflow `imagegen.json` 里的 `api_key_env` 指定。不要在配置文件中保存明文 key；运行时设置对应环境变量。
